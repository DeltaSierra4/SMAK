	...
	for set_type, batch in result_dic.items():
		...
		if set_type.split("_")[1] == "cat":
			for cat, cat_results in result_dic.items():
				...
				wordcloud_gen_cross(cat_results[cat], set_type, cat_dir)
		else:
...
def url_chart_gen(result_dic):
	...
	for set_type, batch in result_dic.items():
		...
		if set_type.split("_")[1] == "cat":
			for cat, cat_results in result_dic.items():
				cat_dir = create_dir(set_dir, cat)
				urlchart_gen_cross(cat_results[cat], set_type, cat_dir)
		else:
...
def stat_chart_gen(result_dic):
	...
	for set_type, batch in result_dic.items():
		...
		if set_type.split("_")[1] == "cat":
			for cat, cat_results in result_dic.items():
				cat_dir = create_dir(set_dir, cat)
				statchart_gen_cross(cat_results[cat], set_type, cat_dir)
		else:
		...
		for time, stats in stat_results.items():
			for stat_type, value in stats.items():
				if "_only" in stat:
					...
					stat_combiner[stat_avg][time] = value[0]
					stat_combiner[stat_avg][time] = value[0]
					stat_combiner[stat_avg][time] = value[0]
					stat_combiner[stat_avg][time] = value[0]
					stat_combiner[stat_avg][time] = value[0]
					...
					if isinstance(value, (int, float)):
						...
						stat_combiner[stat_type][time] = value[0]
